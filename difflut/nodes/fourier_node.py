...
class FourierFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        if not _FOURIER_CUDA_EXT_AVAILABLE:
            raise RuntimeError(
                "Fourier CUDA extension not available. Please compile fourier_cuda extension."
            )
        ...
        if use_eval:
            output = _fourier_cuda_module.forward_eval(
                input, frequencies, amplitudes, phases, bias, max_amplitude
            )
        else:
            output = _fourier_cuda_module.forward(
                input, frequencies, amplitudes, phases, bias, max_amplitude
            )
...
def fourier_forward(
    ...
    if _FOURIER_CUDA_EXT_AVAILABLE and input.is_cuda:
        return FourierFunction.apply(
            input, frequencies, amplitudes, phases, bias, max_amplitude, use_eval
        )
    else:
        ...
        normalized_amplitudes = (
            amplitudes / (amplitudes.sum(dim=0, keepdim=True) + 1e-8) * max_amplitude
        )
...
class FourierNode(BaseNode):
    ...
    def __init__(
        ...
        self.num_frequencies = (
            2 ** self.num_inputs if use_all_frequencies else min(2**self.num_inputs, 32)
        )
    ...
    def forward_train(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if x.is_cuda and self.amplitudes.is_cuda and _FOURIER_CUDA_EXT_AVAILABLE:
