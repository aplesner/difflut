...
def main():
    ...
    print(f"   Data CPU: min={data_cpu.min():.4f}, max={data_cpu.max():.4f}, mean={data_cpu.mean():.4f}")
    print(f"   Data GPU: min={data_gpu.min():.4f}, max={data_gpu.max():.4f}, mean={data_gpu.mean():.4f}")
    ...
    for layer_idx, (layer_cpu, layer_gpu) in enumerate(zip(model_cpu.conv_layers, model_gpu.conv_layers)):
        print(f"\n   Layer {layer_idx}:")
        ...
        for tree_idx in range(min(2, len(layer_cpu.trees))):  # Check first 2 trees
            ...
            for sublayer_idx, (sublayer_cpu, sublayer_gpu) in enumerate(zip(tree_cpu, tree_gpu)):
                ...
                if len(params_cpu) > 0:
                    max_diff = max((p_cpu - p_gpu.cpu()).abs().max().item() 
                                  for p_cpu, p_gpu in zip(params_cpu, params_gpu))
                    print(f"     Sublayer {sublayer_idx}: {len(params_cpu)} params, max_diff = {max_diff:.2e}")
                else:
    ...
    for i in range(output_cpu.shape[0]):
        for j in range(output_cpu.shape[1]):
            if abs(diff[i, j]) > 0.01:
                large_diffs.append((i, j, output_cpu[i,j].item(), output_gpu[i,j].item(), diff[i,j].item()))
