...
def main():
    ...
    node_config = NodeConfig(input_dim=NUM_NODE_INPUTS, output_dim=1)

    layer = layer_class(
        input_size=encoded_dim, output_size=NUM_NODES, node_type=node_class, node_kwargs=node_config
    )
    ...
    plot_results(
        losses, np.array(samples), np.array(labels), model, function_names, device, save_path
    )
